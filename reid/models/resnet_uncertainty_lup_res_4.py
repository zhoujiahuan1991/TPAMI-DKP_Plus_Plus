...
class ResNetSimCLR(nn.Module):
    def __init__(self, base_model='resnet50', out_dim=2048, n_sampling=2, pool_len=8, normal_feature=True,
        ...
        self.pool_len = 8
        self.conv_var =  nn.Conv2d(dim_mlp, dim_mlp, kernel_size=(3,3),bias=False)

        self.n_sampling = n_sampling
        self.n_samples = torch.Size(np.array([n_sampling, ]))
        self.pooling_layer_mean = GeneralizedMeanPoolingP(3)
        self.pooling_layer_var = GeneralizedMeanPoolingP(3)
    ...
    def _get_basemodel(self, model_name):
        ...
        return model
    def load_from_pretrain(self, args):
        from reid.utils.serialization import load_checkpoint, save_checkpoint, copy_state_dict
        if args.pre_train=='pass':
            checkpoint = load_checkpoint('pretrained/pass_vit_base_full.pth')  
        elif args.pre_train=='lup':
            checkpoint = load_checkpoint('pretrained/lup_moco_r50.pth')  
        elif args.pre_train=='lupws':
            checkpoint = load_checkpoint('pretrained/lupws_r50.pth')  
        else:
            checkpoint=None
        if checkpoint is not None:
            copy_state_dict(checkpoint, self.base)     #    
    def forward(self, x, training_phase=None, fkd=False):
        BS = x.size(0)
        
        out = self.base(x)  # former 4 blockes of resnet 50
        out_mean = self.pooling_layer_mean(out)  # global pooling
        ...
        out_mean=F.normalize(out_mean)

        out_var = self.conv_var(out)  # conv layer
        out_var = self.pooling_layer_var(out_var)  # pooling
        out_var += 1e-4
